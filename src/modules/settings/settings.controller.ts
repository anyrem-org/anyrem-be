...
export class SettingsController {
  ...
  @Patch() patch(
    ...
    return this.settings.update(user.id, body.settings as SettingUpdate[]);
  }
  @Put("telegram") async configureTelegram(
    @CurrentUser() user: AuthUser,
    @Body() body: TelegramConfigDto,
  ) {
    if (!body.botToken || !body.chatId)
      throw new BadRequestException("botToken and chatId are required");
    const bot = await this.telegram.getMe(body.botToken);
    await Promise.all([
      ...
      this.settings.setPlain(
        user.id,
        SETTING_TYPES.TELEGRAM,
        SETTING_KEYS.TELEGRAM.BOT_USERNAME,
        bot.username ?? "unknown",
      ),
      this.settings.setPlain(
        user.id,
        SETTING_TYPES.TELEGRAM,
        SETTING_KEYS.TELEGRAM.VERIFIED_AT,
  ...
  @Post("telegram/test") async testTelegram(@CurrentUser() user: AuthUser) {
    ...
    await this.telegram.sendMessage(
      token,
      chatId,
      "Remember Anything: Telegram connection works.",
    );
    return { sent: true };
  }
